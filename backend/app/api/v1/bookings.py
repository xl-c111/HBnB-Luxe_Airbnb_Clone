...
booking_model = api.model(
    "Booking",
    {
        ...
        "total_price": fields.Float(
            readonly=True, description="Total price in USD", example=2500.00
        ),
...
availability_response = api.model(
    "AvailabilityResponse",
    {
        "available": fields.Boolean(
            description="Whether the place is available for the dates", example=True
        ),
...
class BookingList(Resource):
    @api.doc(
        ...
        responses={
            ...
            400: (
                "Invalid input, payment not completed, or place not available",
                error_model,
            ),
    ...
    def post(self):
        ...
        try:
            ...
            if metadata.get("place_id") and metadata.get("place_id") != str(
                booking_data["place_id"]
            ):
                return {
                    "error": "Payment intent does not match the selected place"
                }, 400
            if (
                metadata.get("check_in_date")
                and metadata.get("check_in_date") != booking_data["check_in_date"]
            ):
                return {"error": "Payment intent check-in date mismatch"}, 400
            if (
                metadata.get("check_out_date")
                and metadata.get("check_out_date") != booking_data["check_out_date"]
            ):
