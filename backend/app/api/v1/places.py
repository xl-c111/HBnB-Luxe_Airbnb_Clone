...
place_input_model = api.model(
    "PlaceInput",
    {
        ...
        "price": fields.Float(
            required=True, description="Price per night in USD", example=500.00
        ),
        "latitude": fields.Float(
            required=True, description="Latitude coordinate", example=34.0522
        ),
        "longitude": fields.Float(
            required=True, description="Longitude coordinate", example=-118.2437
        ),
    },
)
...
def serialize_place(place):
    return {
        ...
        "owner_id": (
            str(place.owner.id)
            if (place.owner and hasattr(place.owner, "id"))
            else (str(place.owner_id) if hasattr(place, "owner_id") else None)
        ),
        "amenities": [
            serialize_amenity(a)
            for a in (
                place.amenities
                if hasattr(place.amenities, "__iter__")
                and not isinstance(place.amenities, str)
                else []
            )
        ],
        "reviews": [
            ...
            for r in (
                place.reviews
                if hasattr(place.reviews, "__iter__")
                and not isinstance(place.reviews, str)
                else []
            )
        ],
    }
