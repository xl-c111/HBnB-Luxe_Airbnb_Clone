...
payment_intent_model = api.model(
    "PaymentIntent",
    {
        ...
        "currency": fields.String(
            required=False, description="Currency code (default: usd)"
        ),
    },
)

payment_intent_response = api.model(
...
class CreatePaymentIntent(Resource):
    ...
    def post(self):
        ...
        if not place_id or not check_in_str or not check_out_str:
            return {
                "error": "place_id, check_in_date, and check_out_date are required"
            }, 400
